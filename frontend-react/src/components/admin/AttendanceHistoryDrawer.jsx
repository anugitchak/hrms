...
const AttendanceHistoryDrawer = ({ employee, isOpen, onClose, month }) => {
    ...
    const getStatusConfig = (status) => {
        switch (status) {
            case 'Present': return { bar: 'bg-[#00b9cd]', badge: 'bg-[#00b9cd]/10 dark:bg-[#00b9cd]/10 text-[#00b9cd] dark:text-[#00b9cd] border-[#00b9cd]/10 dark:border-[#00b9cd]/20', icon: <CheckCircle size={11} /> };
                            ...
                            <div className="bg-white dark:bg-slate-900 px-6 py-6 border-b border-slate-100 dark:border-white/5">
                                <div className="flex items-center justify-between mb-6">
                                    ...
                                    <button
                                        type="button"
                                        onClick={onClose}
                                        className="rounded-10 p-2 bg-slate-50 dark:bg-white/5 text-slate-400 hover:text-slate-600 dark:hover:text-white transition-all border border-slate-100 dark:border-white/10 shadow-sm active:scale-95"
                                    >
                                        <X size={20} />
                                    </button>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-3 mb-6">
                                    {[
                                        { label: 'Present', val: presentCount, color: 'text-[#00b9cd]', bg: 'bg-[#00b9cd]/10 dark:bg-[#00b9cd]/10' },
                                        ...
                                        <div key={s.label} className={`${s.bg} rounded-10 p-3 text-center shadow-xl dark:shadow-[0_25px_30px_-5px_rgba(0,0,0,0.7),0_10px_15px_-5px_rgba(0,185,205,0.2)] border border-transparent hover:shadow-2xl dark:hover:shadow-[0_45px_70px_-20px_rgba(0,0,0,0.9),0_10px_15px_-5px_rgba(0,185,205,0.3)] transition-all duration-300 border-2 border-transparent hover:border-[#00b9cd] dark:hover:border-[#00b9cd] transition-all duration-500 ease-out dark:shadow-none border border-slate-100 dark:border-white/5`}>
                                ...
                                <div className="flex gap-3">
                                    <div className="relative flex-1 group">
                                        <input
                                            ...
                                            className="w-full text-xs font-bold p-3 rounded-10 bg-slate-50 dark:bg-white/5 text-slate-900 dark:text-white border-2 border-transparent focus:border-[#00b9cd]/30 outline-none transition-all shadow-inner uppercase"
                                        />
                                    </div>
                                    <div className="relative flex-1 group">
                                        <select
                                            ...
                                            className="w-full text-xs font-bold p-3 rounded-10 bg-slate-50 dark:bg-white/5 text-slate-900 dark:text-white border-2 border-transparent focus:border-[#00b9cd]/30 outline-none transition-all shadow-inner appearance-none uppercase"
                                        >
                            ...
                            <div className="flex-1 overflow-y-auto p-4">
                                {loading ? (
                                    <div className="space-y-3">
                                        {[1,2,3,4].map(i => (
                                            <div key={i} className="animate-pulse bg-slate-50 dark:bg-white/5 rounded-10 h-24"></div>
                                        ))}
                                    </div>
                                ) : error ? (
                                    ...
                                    <ul className="space-y-4">
                                        {filteredHistory.map((record) => {
                                            ...
                                            return (
                                                <li key={record.id} className="relative rounded-10 bg-white dark:bg-slate-800/40 shadow-xl dark:shadow-[0_25px_30px_-5px_rgba(0,0,0,0.7),0_10px_15px_-5px_rgba(0,185,205,0.2)] border border-transparent hover:shadow-2xl dark:hover:shadow-[0_45px_70px_-20px_rgba(0,0,0,0.9),0_10px_15px_-5px_rgba(0,185,205,0.3)] transition-all duration-300 border-2 border-transparent hover:border-[#00b9cd] dark:hover:border-[#00b9cd] transition-all duration-500 ease-out dark:shadow-none border border-slate-100 dark:border-white/5 overflow-hidden hover:-translate-y-1 ">
                                                    {/* Left color bar */}
                                                    ...
                                                    <div className="pl-6 pr-5 py-5">
                                                        ...
                                                        <div className="flex items-center justify-between mb-4">
                                                            ...
                                                            <span className={`flex items-center gap-1 text-[9px] font-black border rounded-10 px-2.5 py-1 uppercase tracking-widest ${sc.badge}`}>
                                                                {sc.icon}
                                                                {record.status}
                                                            </span>
                                                        </div>
                                                        ...
                                                        <div className="grid grid-cols-3 gap-2">
                                                            {[
                                                                { label: 'IN', val: record.check_in || '—', color: record.check_in ? 'text-[#00b9cd]' : 'text-slate-300' },
                                                                { label: 'OUT', val: record.check_out || '—', color: record.check_out ? 'text-[#00b9cd]' : 'text-slate-300' },
                                                                { label: 'DURATION', val: formatDuration(record.total_hours), color: 'text-slate-900 dark:text-slate-100' },
                                                            ].map(col => (
                                                                <div key={col.label} className="bg-slate-50 dark:bg-white/5 rounded-10 p-2.5 text-center shadow-inner border border-slate-50/50 dark:border-white/5">
                                                        ...
                                                        {record.overtime_start && (
                                                            <div className="mt-4 flex items-center gap-3 bg-purple-50 dark:bg-purple-500/10 border border-purple-100 dark:border-purple-500/20 rounded-10 px-3 py-2">
                                                                ...
                                                                <div className="text-[11px] font-black text-purple-700 dark:text-purple-400 uppercase tracking-tight">
                                                                    ...
                                                                    {record.overtime_hours && <span className="text-[#00b9cd] ml-1">({record.overtime_hours}h)</span>}
                                                                </div>
                                                            </div>
                                                        )}
                                                        ...
                                                        {(user?.role_id === 1 || user?.can_force_checkout) && !record.check_out && record.check_in && (
                                                            <div className="mt-4 pt-4 border-t border-slate-50 dark:border-white/5 flex justify-end">
                                                                <button
                                                                    ...
                                                                    className="flex items-center gap-2 text-[10px] font-black px-4 py-2 border-2 border-red-500/20 text-red-600 bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 rounded-10 transition-all shadow-sm active:translate-y-0.5 uppercase tracking-widest"
                                                                >
                                                                    <LogOut size={12} />
                                                                    Force Terminal Out
                                                                </button>
