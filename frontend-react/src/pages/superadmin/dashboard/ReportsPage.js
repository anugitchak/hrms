...
const StatCard = ({ label, value, sub, color = 'teal', icon: Icon }) => { 
    return ( 
        <div className="bg-white dark:bg-slate-900/60 dark:backdrop-blur-md p-6 rounded-[2.5rem] shadow-[4px_4px_0px_0px_rgba(71,85,105,0.3)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.1)] transition-all duration-300 hover:-translate-y-1 hover:shadow-lg border-2 border-slate-50 dark:border-white/5 relative overflow-hidden group">
            {Icon && <Icon className="absolute top-4 right-4 text-teal-500/10 group-hover:scale-110 transition-transform" size={64} />}
...
const LoadingSpinner = () => ( 
    <div className="flex flex-col items-center justify-center py-20 min-h-[400px]"> 
        <Activity size={48} className="text-teal-600 animate-spin mb-4" /> 
...
const ChartTooltip = ({ active, payload, label }) => { 
    ...
    return ( 
        <div className="bg-white dark:bg-slate-900/90 dark:backdrop-blur-md border-2 border-slate-900/5 dark:border-white/10 rounded-[20px] p-5 shadow-[4px_4px_0px_0px_rgba(71,85,105,0.1)]"> 
...
const ReportsPage = () => { 
    ...
    return ( 
        <div className="p-8 max-w-[1600px] mx-auto min-h-screen"> 
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6"> 
                <div> 
                    <h1 className="text-5xl font-black text-slate-900 dark:text-white tracking-tight leading-none mb-3">
                        <span className="italic">Workforce</span> <span className="text-transparent bg-clip-text bg-[#00b9cd]">Analytics</span>
                    </h1>
            ...
            {/* Tab Bar */}
            <div className="flex flex-wrap gap-3 mb-10 p-2 bg-slate-900/5 dark:bg-white/5 rounded-[2rem] w-fit border-2 border-slate-50 dark:border-white/5 backdrop-blur-md">
                {tabs.map(tab => ( 
                    <button 
                        key={tab.id} 
                        onClick={() => setActiveTab(tab.id)} 
                        className={`px-8 py-3.5 text-xs font-black uppercase tracking-widest rounded-[1.5rem] transition-all duration-300 flex items-center gap-2 ${
                            activeTab === tab.id 
                                ? 'bg-teal-600 text-white shadow-[4px_4px_0px_0px_rgba(13,148,136,0.2)] hover:shadow-lg hover:-translate-y-1' 
                    ...
                    {loadingOverview ? <LoadingSpinner /> : ( 
                        <> 
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10"> 
                                <div className="bg-white dark:bg-slate-900/60 dark:backdrop-blur-md p-8 rounded-[2.5rem] shadow-[8px_8px_0px_0px_rgba(71,85,105,0.1)] border-2 border-slate-50 dark:border-white/5"> 
                                    <h3 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-[0.2em] mb-10 flex items-center gap-2"> 
                                        <TrendingUp size={18} className="text-teal-500" /> Completion by Department 
                                    </h3> 
                                ...
                                <div className="bg-white dark:bg-slate-900/60 dark:backdrop-blur-md p-8 rounded-[2.5rem] shadow-[8px_8px_0px_0px_rgba(71,85,105,0.1)] border-2 border-slate-50 dark:border-white/5"> 
                                    <h3 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-[0.2em] mb-10 flex items-center gap-2"> 
                                        <Activity size={18} className="text-teal-500" /> Status Breakdown 
                                    </h3> 
                                    <div className="h-[350px]"> 
                                        <ResponsiveContainer width="100%" height="100%"> 
                                ...
                                <div className="space-y-6"> 
                                    {(analytics?.categories || []).map((cat, idx) => ( 
                                        <div key={idx} className="group"> 
                                            ...
                                            <div className="w-full bg-slate-100 dark:bg-white/5 rounded-full h-4 p-1 border border-slate-200 dark:border-white/10"> 
                                                <div className="h-full rounded-full transition-all duration-1000 group-hover:shadow-[0_0_15px_rgba(13,148,136,0.3)] shadow-inner" style={{ width: `${Math.min((cat.total / Math.max(...(analytics?.categories || []).map(c => c.total), 1)) * 100, 100)}%`, background: `linear-gradient(90deg, ${COLORS[idx % COLORS.length]}, #10b981)` }} /> 
                                            </div> 
                                        </div> 
                                    ))} 
                                    {(!analytics?.categories || analytics.categories.length === 0) && ( 
                                        <div className="h-40 flex flex-col items-center justify-center text-slate-400">
                                            ...
                                            <p className="text-[10px] font-black uppercase tracking-widest italic">No data streams detected...</p> 
                                        </div>
                                    )} 
                                </div> 
                            </div> 
            ...
            {activeTab === 'employee' && ( 
                <div className="space-y-8"> 
                    {loadingPerf ? <LoadingSpinner /> : ( 
                        <> 
                            <div className="bg-white dark:bg-slate-900/60 dark:backdrop-blur-md p-8 rounded-[2.5rem] shadow-[8px_8px_0px_0px_rgba(71,85,105,0.1)] border-2 border-slate-50 dark:border-white/5 flex flex-wrap items-center justify-between gap-8"> 
                                <div className="flex-1 min-w-[300px]"> 
                                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4">Select Operative</p> 
                                    <select value={selectedEmployeeId} onChange={e => setSelectedEmployeeId(e.target.value)} className="w-full px-6 py-4 bg-slate-50 dark:bg-white/5 border-2 border-slate-200 dark:border-white/10 rounded-2xl text-slate-900 dark:text-white text-xs font-black uppercase tracking-widest outline-none focus:border-teal-500 transition-colors cursor-pointer appearance-none shadow-inner" > 
                                ...
                                {selectedEmployee && ( 
                                    <div className="flex items-center gap-6 bg-teal-50 dark:bg-teal-500/10 p-6 rounded-[2rem] border-2 border-teal-100 dark:border-teal-500/20 shadow-md"> 
                                        <div className="w-14 h-14 rounded-2xl bg-teal-600 shadow-[4px_4px_0px_0px_rgba(13,148,136,0.3)] flex items-center justify-center text-white font-black text-xl font-paperlogy"> {selectedEmployee.name.charAt(0).toUpperCase()} </div> 
                                        <div> 
                                            <p className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest">{selectedEmployee.name}</p> 
                                            <p className="text-[10px] text-teal-600 font-bold uppercase tracking-widest mt-1">{selectedEmployee.department}</p> 
                                        </div> 
                                        <div className="ml-8 text-center px-6 py-3 bg-white dark:bg-slate-900/80 rounded-2xl shadow-inner border border-teal-100/50">
                                            <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Efficiency Ratio</p>
                                            <span className="text-xl font-black text-teal-600">{selectedEmployee.completion_rate}%</span> 
                                        </div>
                                    </div> 
                                )} 
                            </div> 
                            {selectedEmployee ? ( 
                                <> 
                                    ...
                                    <div className="bg-white dark:bg-slate-900/60 dark:backdrop-blur-md p-8 rounded-[2.5rem] shadow-[8px_8px_0px_0px_rgba(71,85,105,0.1)] border-2 border-slate-50 dark:border-white/5 group"> 
                                        ...
                                        <div className="flex items-center gap-6"> 
                                            <div className="flex-1 bg-slate-100 dark:bg-white/5 rounded-full h-8 p-1.5 border border-slate-200 dark:border-white/10"> 
                                                <div className="h-full rounded-full transition-all duration-1000 group-hover:shadow-[0_0_20px_rgba(13,148,136,0.3)] shadow-inner" style={{ width: `${selectedEmployee.completion_rate}%`, background: `linear-gradient(90deg, #0d9488, #10b981)` }} /> 
                                            </div> 
                                            <span className="font-paperlogy font-black text-teal-600 text-3xl w-24 text-right"> {selectedEmployee.completion_rate}% </span> 
                                        </div> 
                                ...
                                <div className="bg-white dark:bg-slate-900/60 dark:backdrop-blur-md p-20 rounded-[2.5rem] shadow-lg border-2 border-dashed border-slate-200 dark:border-white/10 text-center"> 
                            ...
                            <div className="bg-white dark:bg-slate-900/60 dark:backdrop-blur-md p-8 rounded-[2.5rem] shadow-[12px_12px_0px_0px_rgba(71,85,105,0.1)] border-2 border-slate-900/5 dark:border-white/5 overflow-hidden"> 
                                    ...
                                    <table className="w-full"> 
                                        <thead> 
                                            <tr className="bg-slate-900 text-[10px] text-white font-black uppercase tracking-[0.2em]"> 
                                                <th className="px-6 py-5 text-left rounded-tl-[1.5rem]">Rank</th> 
                                                ...
                                                <th className="px-6 py-5 text-center rounded-tr-[1.5rem]">Ratio</th> 
                                            </tr> 
                                        </thead> 
                                        <tbody className="divide-y divide-slate-50 dark:divide-white/5"> 
                                            {topPerformers.map((emp, idx) => ( 
                                                <tr key={emp.id} onClick={() => { setSelectedEmployeeId(emp.id); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="hover:bg-teal-50/50 dark:hover:bg-teal-500/5 cursor-pointer transition-colors group" > 
                                                    <td className="px-6 py-5 font-black text-slate-900 dark:text-white text-xs"> 
                                                        {idx < 3 ? ['🥇', '🥈', '🥉'][idx] : `#${idx + 1}`} 
                                                    </td> 
                                                    <td className="px-6 py-5"> 
                                                        <div className="flex items-center gap-3"> 
                                                            <div className="w-9 h-9 rounded-xl bg-teal-600 flex items-center justify-center text-white text-xs font-black shadow-md"> {emp.name.charAt(0)} </div> 
                                                            <span className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-widest group-hover:text-teal-600 transition-colors">{emp.name}</span> 
                                                        </div> 
                                                    </td> 
                                                    ...
                                                    <td className="px-6 py-5 text-center font-bold text-teal-600 text-xs">{emp.completed}</td> 
                                                    <td className="px-6 py-5 text-center font-bold text-slate-400 text-xs">{emp.pending}</td> 
                                                    <td className="px-6 py-5 text-center"> 
                                                        <span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest ${emp.completion_rate >= 75 ? 'bg-emerald-500 text-white' : emp.completion_rate >= 40 ? 'bg-amber-500 text-white' : 'bg-rose-500 text-white' } shadow-sm`}> {emp.completion_rate}% </span> 
                                                    </td> 
                                                </tr> 
                                            ))} 
                                            {topPerformers.length === 0 && ( 
                                                <tr> <td colSpan={7} className="text-center py-20 text-slate-400 font-bold uppercase tracking-widest italic"> Systems isolated... No performance streams. </td> </tr> 
                                            )} 
                                        </tbody> 
                                    </table> 
                                </div> 
                ...
                <div className="space-y-10"> 
                    {loadingPerf ? <LoadingSpinner /> : ( 
                        <> 
                            <div className="bg-white dark:bg-slate-900/60 dark:backdrop-blur-md p-8 rounded-[2.5rem] shadow-[8px_8px_0px_0px_rgba(71,85,105,0.1)] border-2 border-slate-50 dark:border-white/5"> 
                                <h3 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-[0.2em] mb-10 flex items-center gap-2"> 
                                    <Activity size={18} className="text-teal-500" /> Industrial Output — 6-Month Trajectory 
                                </h3> 
                            ...
                            <div className="bg-white dark:bg-slate-900/60 dark:backdrop-blur-md p-8 rounded-[2.5rem] shadow-[8px_8px_0px_0px_rgba(71,85,105,0.1)] border-2 border-slate-50 dark:border-white/5"> 
                                ...
                                <div className="flex flex-wrap items-center gap-8 mt-10 justify-center"> 
                                    <span className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-500"><span className="w-4 h-4 rounded-lg bg-emerald-500 shadow-md shadow-emerald-500/30" />Optimal Output (≥ 75%)</span> 
                                    <span className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-500"><span className="w-4 h-4 rounded-lg bg-amber-500 shadow-md shadow-amber-500/30" />Sustained Output (40–74%)</span> 
                                    <span className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-500"><span className="w-4 h-4 rounded-lg bg-rose-500 shadow-md shadow-rose-500/30" />Sub-nominal Output (&lt; 40%)</span> 
                                </div> 
                            </div> 
